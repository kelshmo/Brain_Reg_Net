---
title: "Covariate analysis of CMC RNASeq data reprocessed with RSEM-feature counts workflow with GRCh38 (ACC and DLPFC with CQN normlaisation)"
...
knit2synapse::createAndKnitToFolderEntity(file = "./CMC_ACC_DLPFC_RSEM_GRCh38_CQN_ModSel.Rmd",
                                 parentId ="syn9846623",
                                 folderName = 'ACC and DLPFC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model')
```
...
activityDescription = 'Covariate analysis of RSEM aligned effective counts with GRCh38 with CQN normalisation (ACC and DLPFC)';

thisFileName <- 'CMC_ACC_DLPFC_RSEM_GRCh38_CQN_ModSel.Rmd'
...
COUNT_ACC_ID = 'syn16783564'
ALL_USED_IDs = COUNT_ACC_ID
COUNT_ACC = downloadFile(COUNT_ACC_ID) %>% data.frame()

# Download gene lengths (ACC)
GENE_LEN_ID = 'syn16783573'
GENE_LEN_ACC = downloadFile(GENE_LEN_ID) %>%
  tidyr::gather(sampleID, Length, -gene_id, -`transcript_id(s)`) %>%
  group_by(gene_id) %>%
  summarise(Length = median(Length, na.rm = T)) %>%
  ungroup() %>% data.frame()
ALL_USED_IDs = c(ALL_USED_IDs, GENE_LEN_ID)

# Download reprocessed counts (DLPFC)
COUNT_DLPFC_ID = 'syn16783526'
COUNT_DLPFC = downloadFile(COUNT_DLPFC_ID) %>% data.frame()
ALL_USED_IDs = c(ALL_USED_IDs, COUNT_DLPFC_ID)

# Download gene lengths (DLPFC)
GENE_LEN_ID = 'syn16783530'
GENE_LEN_DLPFC = downloadFile(GENE_LEN_ID) %>%
  tidyr::gather(sampleID, Length, -gene_id, -`transcript_id(s)`) %>%
  group_by(gene_id) %>%
  summarise(Length = median(Length, na.rm = T)) %>%
  ungroup() %>% data.frame()
...
GENE.LEN = bind_rows(GENE_LEN_ACC,GENE_LEN_DLPFC) %>% 
  group_by(gene_id) %>% 
...
ALL_USED_IDs[length(ALL_USED_IDs)+1] = METADATA_QC_ID
METADATA_QC = downloadFile_version(METADATA_QC_ID, version = 13)  %>% 
...
# metadataVarOfInterest = c("Individual_ID", "Institution", "Reported_Gender", "Age_of_Death", "PMI_(in_hours)", "Dx", 
#                       
#                           "rnaSeq_isolation:Exclude?", "Sample_RNA_ID",
#                           "rnaSeq_report:RIN", "rnaSeq_report:Exclude?",
...
DE = DE %>% 
  ...
  left_join(Ensemble2HGNC) %>%
  left_join(GENE.LEN %>% rownameToFirstColumn('gene_id'))
...
# Code
CODE <- Folder(name = "ACC and DLPFC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)
