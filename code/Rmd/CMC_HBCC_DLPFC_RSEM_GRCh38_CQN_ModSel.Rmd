...
METADATA_QC_DLPFC = downloadFile_version(METADATA_QC_DLPFC_ID, version = 10) %>%
...
METADATA = METADATA_QC_DLPFC %>%
  ...
  dplyr::rename(PMI = `PMI_(in_hours)`,
                ...
                TotalReads = `rnaSeq_report:Total_Reads`, AlignmentRate = `rnaSeq_report:Percent_Aligned`) %>%
...
p = p + geom_point(aes(color=`Reported Gender`)) + 
  ggtitle("Sex Check MSSM/Penn/Pitt Cohort") + 
