---
title: "Covariate analysis of CMC RNASeq data reprocessed with RSEM-feature counts workflow with GRCh38 (HBCC DLPFC with CQN normlaisation)"
...
knit2synapse::createAndKnitToFolderEntity(file = "./CMC_HBCC_DLPFC_RSEM_GRCh38_CQN_ModSel.Rmd",
                                          parentId ="syn9846623",
                                          folderName = 'HBCC - DLPFC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model')
```
...
activityDescription = 'Covariate analysis of RSEM aligned effective counts with GRCh38 with CQN normalisation (HBCC DLPFC)';

thisFileName <- 'CMC_HBCC_DLPFC_RSEM_GRCh38_CQN_ModSel.Rmd'
...
# #featureCounts
# COUNT_ID = 'syn16783066'
# ALL_USED_IDs = COUNT_ID
# COUNT = downloadFile(COUNT_ID) %>% 
#   data.frame() %>% 
#   select(-one_of("Chr","Start","End","Strand","Length"))
# rownames(COUNT) = COUNT$Geneid
# COUNT$Geneid = NULL

# Download gene lengths (DLPFC)
GENE.LEN = downloadFile('syn16783074') %>% 
...
METADATA_QC_DLPFC = downloadFile_version(METADATA_QC_DLPFC_ID, version = 12) %>%
...
METADATA = METADATA_QC_DLPFC %>%
  ...
  dplyr::select(SampleID, Individual_ID, Institution, Reported_Gender,Sex, Ethnicity, Age_of_Death, PMI, Dx, 
                RIN, IsolationExclude, ReportExclude, GenotypeExclude, EV.1, EV.2, EV.3, EV.4, EV.5,
...
ind = METADATA$SampleID[which(METADATA$IsolationExclude == 1 | METADATA$ReportExclude == 1 | METADATA$GenotypeExclude)]
...
indToRemove = c("CMC_HBCC_RNA_PFC_3402", "CMC_HBCC_RNA_PFC_3370","CMC_HBCC_RNA_PFC_3152","CMC_HBCC_266","CMC_HBCC_RNA_PFC_3405", "CMC_HBCC_RNA_PFC_3208")
...
p <- p + geom_point(aes(shape=Dx, size=Age_of_Death))
...
phase1.covars = c( 'Reported_Gender', 'PMI', 'Age_of_Death', 'RIN', 'RIN2', 'EV.1', 'EV.2', 'EV.3', 'EV.4', 'EV.5')
...
# Eucledian tree based analysis
COVARIATES.tmp = data.matrix(as.numeric(COVARIATES[,c('Dx')])) 
rownames(COVARIATES.tmp) = rownames(COVARIATES)
colnames(COVARIATES.tmp) <- c('Dx')
COVARIATES.tmp[is.na(COVARIATES.tmp)] = 0
...
# Code
CODE <- Folder(name = "HBCC - DLPFC - RSEM - EXPECTED COUNTS - GRCh38 - CQN - Sparse Model", parentId = parentId)
CODE <- synStore(CODE)
